"""Error metrics and confidence-interval helpers shared with the notebooks."""
import numpy as np

# z-scores for common two-sided confidence levels, used to turn a residual
# std into a +/- band around a point forecast
...
def confidence_band(forecast, actual, confidence_level: int):
    """Build a +/- band around `forecast` sized from the forecast's own residual std.

    Using the residual std observed over the backtest window (rather than a
    fixed % of the forecast) means the band reflects how reliable *this*
    model actually was for *this* category, not a generic guess.
    """
